    ...
    <div className="bg-gray-800 border border-gray-700 rounded p-4 h-[560px] flex flex-col text-gray-100">
            ...
            <div className="mt-2 text-sm">
              {ticketClasses.map((tc) => (
                <div key={tc.id} className="flex justify-between py-1 border-b border-gray-700">
                  ...
                  <div className="font-medium">{(tc.price || 0).toLocaleString()} đ</div>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="mt-2 text-sm space-y-1">
            ...
            {selectedSeats.map((s) => (
              <div key={s.id} className="flex justify-between py-1 border-b border-gray-700">
                ...
                <div className="font-medium">{(s.price || 0).toLocaleString()} đ</div>
              </div>
            ))}
          </div>
        </div>
      ...
      <div className="mt-3">
        <div className="flex justify-between text-sm pb-3 border-t border-gray-700 pt-3">
          <div>Subtotal</div>
          <div className="font-semibold">{subtotal.toLocaleString()} đ</div>
        </div>
        <button
          onClick={onContinue}
...
const OrderSeat = () => {
      ...
      try {
        ...
        if (mounted) {
          setEventData({
            ...
            date: concert?.start_time ? new Date(concert.start_time).toLocaleDateString() : concert?.date,
            start: concert?.start_time ? new Date(concert.start_time).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }) : concert?.start,
            end: concert?.end_time ? new Date(concert.end_time).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }) : concert?.end,
        ...
        const layoutArray = Object.values(rows).sort((a, b) => a.row.localeCompare(b.row));
  ...
  useEffect(() => {
    ...
    selectedSeats.forEach((s) => {
      ...
      if (!groups[tcId]) groups[tcId] = { ticketClassId: tcId, ticketClassName: s.ticketClass || tcId, price: s.price || 0, seats: [] };
  ...
  return (
    <div className="container mx-auto px-4 pt-4 pb-6 bg-gray-900 text-gray-100 min-h-screen">
      ...
      <div className="mb-3 relative">
        <button onClick={() => navigate(-1)} className="absolute left-0 text-gray-300 hover:text-white">← Quay lại</button>
        <h1 className="text-2xl font-semibold text-center">Chọn chỗ ngồi</h1>
      </div>
      ...
      <div className="flex items-start gap-6">
        <div className="w-1/2">
          ...
          <div className="mt-3 p-3 bg-gray-800 border border-gray-700 rounded text-sm text-gray-200">
            <div className="mb-2">Chọn vé:</div>
