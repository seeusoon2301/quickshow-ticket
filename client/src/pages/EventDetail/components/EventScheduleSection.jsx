...
const formatTime = (dateObj) => {
  if (!dateObj || !(dateObj instanceof Date) || isNaN(dateObj.getTime())) return 'Chưa cập nhật';
  ...
  return (
    <section id="section-schedule" className="px-6 md:px-16 lg:px-24 py-10 border-t border-white/10">
      <div className="flex items-center justify-between mb-6">
        <h2 className="text-2xl font-bold text-white">Lịch diễn</h2>
    ...
    <div className="text-center py-8">
      ...
      <div className="mx-auto w-3/4 max-w-md mb-8">
        <div className="bg-gradient-to-b from-gray-700 to-gray-800 h-12 rounded-t-full flex items-center justify-center">
          <span className="text-gray-300 font-semibold tracking-widest text-sm">SÂN KHẤU</span>
        </div>
      </div>
        ...
        {ticketClasses.length > 0 ? (
          ticketClasses.map((tc, idx) => (
            ...
            >
              ...
              <span className="text-gray-400 ml-2">- {formatPrice(tc.price)}đ</span>
            </div>
          ))
        ) : (
          ...
          <div key={show.id} className="border border-white/10 rounded-lg overflow-hidden">
              ...
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-primary text-sm">
                    {showDate.getDate()} Tháng {String(showDate.getMonth() + 1).padStart(2, '0')}, {showDate.getFullYear()}
                  </p>
                </div>
              </div>
              <button
                onClick={(e) => {
                  e.stopPropagation();
                  onBuyTicket(ticketClasses[0], show);
                }}
                className="px-6 py-2 bg-primary hover:bg-primary/90 text-black font-semibold rounded-lg transition"
              >
                Mua vé ngay
              </button>
            </div>
            ...
            {isExpanded && (
              <div className="border-t border-white/10">
                <div className="p-4">
                  <h4 className="text-gray-400 text-sm mb-3">Thông tin vé</h4>
      ...
      <div className="flex items-center justify-center gap-2 mb-4 overflow-x-auto pb-2 scrollbar-hide">
        ...
        {monthSelector.slice(0, 5).map((month, idx) => (
          ...
          >
            ...
            <p className="text-gray-400 text-xs">{month.showCount} suất diễn</p>
          </div>
        ))}
            ...
            {getShowsForDate(selectedDate).map((show) => (
              <div key={show.id} className="mb-6">
                <div className="flex items-center justify-between mb-4">
                  ...
                  <button
                    onClick={() => onBuyTicket(ticketClasses[0], show)}
                    className="px-4 py-2 bg-primary hover:bg-primary/90 text-black font-semibold rounded-lg transition"
                  >
                    Mua vé ngay
                  </button>
                </div>
                ...
                <div className="space-y-3">
                  <h4 className="text-gray-400 text-sm">Thông tin vé</h4>
                  {ticketClasses.map((tc) => {
                    const isAvailable = (tc.seatStats?.available || tc.available_qty || tc.quota - tc.sold_qty) > 0;
