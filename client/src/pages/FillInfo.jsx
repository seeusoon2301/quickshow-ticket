...
const FillInfo = () => {
  ...
  const handleProceedPayment = () => {
    ...
    navigate('/payment', { state: { cartItems, seats: seatsFromState, event, customer: { fullName, email, phone, idNumber } } });
  };
    ...
    <div className="min-h-screen bg-gray-900 text-gray-100 pt-16">
      <div className="container mx-auto px-4 py-6">
        ...
        <div className="mb-4 flex items-center justify-between">
          <button onClick={() => navigate(-1)} className="text-gray-300 hover:text-white">← Quay lại</button>
          <div className="text-sm text-gray-400">Bước 2 / 3</div>
        </div>
        ...
        <div className="flex gap-6">
          ...
          <div className="w-2/3">
            <div className="bg-gray-800 border border-gray-700 rounded p-4 mb-4">
                <div className="mb-2 text-sm text-gray-300">Tóm tắt đơn hàng</div>
              <div className="mt-4 bg-gray-900 border border-gray-800 rounded p-3">
                <h3 className="font-medium mb-2">Thông tin nhận vé</h3>
                <div className="text-sm text-gray-300">Tên khách hàng: <strong className="text-gray-100">{fullName || '—'}</strong></div>
                <div className="text-sm text-gray-300 mt-1">Email: <strong className="text-gray-100">{email || '—'}</strong></div>
                <div className="text-sm text-gray-300 mt-1">Hình thức nhận vé: <strong className="text-gray-100">{deliveryMethod === 'email' ? 'Gửi email' : 'Nhận tại quầy'}</strong></div>
              </div>

              <div className="mt-4 bg-gray-900 border border-gray-800 rounded p-3">
                <h3 className="font-medium mb-2">Khu vực và ghế ngồi</h3>
                {cartItems && cartItems.length > 0 ? (
                  cartItems.map((it) => (
                    <div key={it.id} className="py-2 border-b border-gray-800">
                      ...
                      <div className="flex justify-between text-sm mb-1">
                        <div>{it.ticketClassName} · {it.seats ? it.seats.length : it.quantity} vé</div>
                      ...
                      {it.seats && it.seats.map((s) => (
                        <div key={s.id} className="text-xs text-gray-400">Row {s.row} - {s.number} — {(s.price || it.price || 0).toLocaleString()} đ</div>
                      ))}
                    </div>
                  ))
                  ...
                  seatsFromState.map((s) => (
                    <div key={s.id} className="py-2 border-b border-gray-800 text-sm flex justify-between">
                      ...
                      <div className="font-semibold">{(s.price || 0).toLocaleString()} đ</div>
                    </div>
                  ))
                ) : (
                  <div className="text-gray-400">Chưa chọn ghế</div>
                )}

                <div className="mt-3 flex items-center justify-between border-t border-gray-800 pt-3">
                  <div className="text-sm text-gray-300">Tổng</div>
                  <div className="text-lg font-semibold">{totals.total.toLocaleString()} đ</div>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="w-1/3">
            <div className="bg-gray-800 border border-gray-700 rounded p-4">
              <h3 className="font-semibold mb-3">Thông tin người đại diện nhận vé</h3>
              ...
              <input value={phone} onChange={(e) => {
                ...
                else if (!isValidPhone(v)) setPhoneError('Số điện thoại không đúng định dạng, vui lòng thử lại.');
                else setPhoneError('');
              ...
              <div className="mt-2">
                <div className="text-sm text-gray-300 mb-2">Hình thức nhận vé</div>
                <div className="flex flex-col gap-2">
                  <label className={`p-2 rounded border ${deliveryMethod === 'email' ? 'border-indigo-500' : 'border-gray-700'} cursor-pointer`}>
                    <input type="radio" name="delivery" value="email" checked={deliveryMethod === 'email'} onChange={() => setDeliveryMethod('email')} className="mr-2" /> Gửi vé qua email
                  </label>
                  <label className={`p-2 rounded border ${deliveryMethod === 'counter' ? 'border-indigo-500' : 'border-gray-700'} cursor-pointer`}>
                    <input type="radio" name="delivery" value="counter" checked={deliveryMethod === 'counter'} onChange={() => setDeliveryMethod('counter')} className="mr-2" /> Nhận vé tại quầy
                  </label>
                </div>
              </div>

              <div className="mt-4 text-xs text-gray-400 mb-2">Vui lòng nhập số điện thoại để tiếp tục (bắt buộc).</div>
              <div className="mt-2 flex gap-2">
                <button onClick={() => navigate(`/order/${event?._id || (cartItems[0]?.eventId) || ''}`)} className="flex-1 bg-gray-700 text-white py-2 rounded">Chọn lại vé</button>
                <button
                  disabled={!(hasTickets && isValidPhone(phone))}
                  onClick={() => {
                    if (!isValidPhone(phone)) {
                      setPhoneError('Số điện thoại không đúng định dạng, vui lòng thử lại.');
                      return;
                    }
