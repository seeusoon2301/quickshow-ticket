...
const TheaterAndArtPage = () => {
  ...
  useEffect(() => {
    fetch("http://localhost:5000/events")
      .then((res) => res.json())
      .then((data) => {
        const theaters = data.filter((e) => e.category === "theater");
        setTimeout(() => { // delay để skeleton hiển thị
          setTheaterItems(theaters);
          setLoading(false);
        }, 1500);
      })
      .catch((err) => {
          ...
          : theaterItems.map((item) => (
              ...
              >
                ...
                <div className="flex items-center justify-between text-gray-400 px-3 pb-3">
                  ...
                  <p className="text-primary font-bold">{item.price_set}đ</p>
                </div>
              </div>
            ))}
      </div>
...
export default TheaterAndArtPage;
