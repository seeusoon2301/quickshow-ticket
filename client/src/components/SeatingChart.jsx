...
const SeatingChart = ({ concertId, initialSelected = [], onChange, scale = 1 }) => {
  ...
  useEffect(() => {
    ...
    const fetchSeats = async () => {
      try {
        const res = await api.ticket.getSeats(concertId);
  ...
  const handleSelect = async (seat) => {
    ...
    // Try to lock via API
    try {
      const res = await api.ticket.lockSeats(concertId, [sid]);
