...
const FilterBar = ({ 
  ...
  const formatDateDisplay = () => {
    if (filters.startDate && filters.endDate) {
      return `${new Date(filters.startDate).toLocaleDateString("vi-VN")} - ${new Date(filters.endDate).toLocaleDateString("vi-VN")}`;
    }
    if (filters.startDate) {
      return `Từ ${new Date(filters.startDate).toLocaleDateString("vi-VN")}`;
    }
    return "Chọn ngày";
  };
    ...
    <div className="bg-[#252525] rounded-xl p-4 mb-6 animate-fadeIn">
      <div className="flex flex-wrap items-center gap-3">
        ...
        <div className="relative" ref={cityRef}>
          ...
          >
            <MapPin size={16} />
            <span className="text-sm">{filters.city || "Tất cả thành phố"}</span>
